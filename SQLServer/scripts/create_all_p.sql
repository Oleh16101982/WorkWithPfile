   ...
   select @script_d='CREATE PROCEDURE [P_'+@tablename+'_D] '
	...
	select @CheckPart='	IF dbo.F_CheckTriggers() = 1
				BEGIN
					...
					EXECUTE p_Log_I @ErrorNumber , @ErrorSeverity , @ErrorState , '''+@tablename+''' , @ErrorNumber , 0 , @ErrorText
...
DROP TABLE #sp_Caller'
	...
	/*select @script_i*/
	if object_id('P_'+@tablename+'_I') is  NULL 
		exec(@script_i)
	if (object_id('P_'+@tablename+'_U') is  NULL) and (@tablename<>'Log')
		exec(@script_u)
	if (object_id('P_'+@tablename+'_D') is  NULL) and (@tablename<>'Log')
		exec(@script_d)
